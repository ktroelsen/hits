...
export function VictoryModal({ isOpen, winner, onRestart, onExit, onPlaySong }: VictoryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-charcoal-950/85 backdrop-blur-md animate-in fade-in duration-300">
      <div className="relative w-full max-w-2xl bg-charcoal-900 border-2 border-amber-500/50 rounded-3xl p-6 sm:p-8 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div className="absolute -bottom-24 -right-24 w-60 h-60 bg-coral-500/20 rounded-full blur-3xl pointer-events-none" />

        {/* Top Celebration Banner */}
        <div className="text-center relative z-10">
          <div className="inline-flex p-3 sm:p-4 rounded-3xl bg-gradient-to-tr from-amber-500 to-yellow-300 text-charcoal-950 shadow-xl shadow-amber-500/30 mb-3 animate-bounce">
        ...
        <div className="mt-6 flex-1 overflow-y-auto pr-1 space-y-2 max-h-60 sm:max-h-72">
          <div className="flex items-center justify-between text-xs text-charcoal-400 font-bold uppercase tracking-wider px-2 mb-2">
          ...
          {winner.timeline.map((song, i) => (
            <div
              key={song.id}
              className="flex items-center justify-between p-3 rounded-2xl bg-charcoal-950/60 border border-charcoal-800 hover:border-charcoal-700 transition-colors group"
            >
              <div className="flex items-center gap-3">
                <span className="w-6 text-center text-xs font-mono text-charcoal-500 font-bold">
                  #{i + 1}
                </span>
                ...
                <div className="flex flex-col">
                  <span className="text-xs sm:text-sm font-bold text-charcoal-100 group-hover:text-coral-300 transition-colors">
                    {song.title}
                  </span>
              ...
              <button
                ...
                className="p-2 rounded-xl bg-charcoal-800 hover:bg-coral-600 text-charcoal-300 hover:text-white transition-all shadow-sm"
        ...
        {/* Footer Actions */}
        <div className="mt-6 pt-4 border-t border-charcoal-800 flex items-center justify-between flex-wrap gap-3">

...
export function Standings({
  ...
  return (
    <div className="mt-6">
      <p className="mb-3 text-center text-sm font-bold uppercase tracking-widest text-slate-400">{title}</p>
          ...
          return (
            <motion.div
              ...
              className={`flex items-center justify-between rounded-lg px-4 py-3 ${
                i === 0 ? 'bg-amber-500/15 ring-1 ring-amber-500/60' : 'bg-slate-800/70'
              } ${p.id === meId ? 'ring-2 ring-pink-500' : ''}`}
            >
              <span className="font-semibold">
                <span className="mr-2 text-slate-500">{i + 1}.</span>
                <span style={{ color: p.color }}>{p.name}</span>
                {p.id === meId && <span className="ml-1 text-xs text-slate-400">(dig)</span>}
              </span>
              <span className="flex items-center gap-3">
                <span className={`text-sm font-bold ${points > 0 ? 'text-emerald-400' : 'text-slate-500'}`}>
                  +{points}
                </span>
